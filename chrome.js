...
function Header({ onJump, transparent }) {
  ...
  return (
    <header style={{
      ...
      background: onDark ? 'transparent' : 'rgba(250,247,241,0.86)',
      ...
      <div style={{ maxWidth: 'var(--container-max)', margin: '0 auto', padding: '0 var(--space-7)',
        height: 76, display: 'grid', gridTemplateColumns: '1fr auto 1fr', alignItems: 'center' }}>
        <nav style={{ display: 'flex', gap: 'var(--space-6)' }}>
          <button onClick={() => onJump('work')} style={{ ...linkStyle, color: fg }}>Work</button>
          <button onClick={() => onJump('atelier')} style={{ ...linkStyle, color: fg }}>Atelier</button>
        </nav>
        <button onClick={() => onJump('top')} style={{
          fontFamily: 'var(--font-display)', fontWeight: 400, fontSize: 22, letterSpacing: '0.3em',
          textTransform: 'uppercase', color: fg, background: 'none', border: 'none', cursor: 'pointer', paddingLeft: '0.3em' }}>Andolini</button>
        <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
          <button onClick={() => onJump('contact')} style={{ ...linkStyle, color: fg }}>Contact</button>
        </div>
      </div>
    </header>
  );
...
/* ════════════════════════════════════════════════════════════════════
   ...
   ──────────────────────────────────────────────────────────────────
   1) ADDRESSES — replace the placeholder shops with your real ones.
      Tapping an address opens it in the 2GIS app/site; a second small
      link opens Google Maps. The map links are built automatically from
      `street` + `zip`, so just keep those accurate.
   2) INSTAGRAM / EMAIL — set your real handle + email just below.
   ════════════════════════════════════════════════════════════════════ */
...
// ✦ Your social + email — REPLACE the placeholders below ✦
const CONTACT = {
  instagram: 'andolini',            // ← your Instagram handle (no @). Becomes instagram.com/andolini
  email:     'orders@andolini.it',  // ← your email
};

// Builds map links from an address. 2GIS = the app you asked for; Google as a backup.
const mapsUrl = (a) => ({
  ...
  return (
    <footer id={id} style={{ background: 'var(--ink-900)', color: 'var(--sand-50)',
      ...
      <div style={{ maxWidth: 'var(--container-narrow)', margin: '0 auto', textAlign: 'center' }}>
        ...
        <h2 style={{ fontSize: 'clamp(34px,5vw,64px)', fontWeight: 400, lineHeight: 1.05, color: 'var(--sand-50)',
      ...
      <div style={{ maxWidth: 'var(--container-max)', margin: 'var(--space-9) auto 0', display: 'grid',
        gridTemplateColumns: 'repeat(4, 1fr)', gap: 'var(--space-6)' }} className="kit-addresses">
        {ADDRESSES.map((a) => (
          <div key={a.city} style={{ borderTop: '1px solid var(--ink-700)', paddingTop: 'var(--space-5)',
            display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
            <div style={{ fontFamily: 'var(--font-display)', fontWeight: 400, fontSize: 18, letterSpacing: '0.16em',
              textTransform: 'uppercase', color: 'var(--sand-400)', marginBottom: 'var(--space-2)' }}>{a.city}</div>
            {/* Address opens in the 2GIS app/site */}
            ...
            {/* Secondary map link → Google Maps */}
      ...
      <div style={{ maxWidth: 'var(--container-narrow)', margin: 'var(--space-9) auto 0', textAlign: 'center' }}>
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 'var(--space-7)', flexWrap: 'wrap' }}>
      ...
      <div style={{ maxWidth: 'var(--container-max)', margin: '0 auto', marginTop: 'var(--space-10)',
